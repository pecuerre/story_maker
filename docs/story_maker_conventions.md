# Story Maker Rails App - Conventions Summary

## Core Patterns

### Database Schema
- All main models have: `name`, `description`, `story_id` (FK), `parent_id` (self-ref FK), `position` (order), created_at/updated_at
...
- Custom validators check story_id consistency across relationships
...
- Story scoping via Current.story

### Routes
- Nested under `scope "s/:story_slug", as: :story`
...
### Navigation (Sidebar)
- Organized by story theme questions:
...
3. Nested route under story scope: `resources :events`
