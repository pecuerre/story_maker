...
`2026_09_24_120000`) and the models in `app/models/`.
...
| `universes` | `name`, `owner_id` FK→users, `private` (bool, default `false`), `slug` (unique) | `visible_to(user)` includes public universes plus private universes owned by or joined by the user |
...
| Model | Validations |
|---|---|
| `Universe` | `name` and owner presence; slug generated by `HasSlug` |
