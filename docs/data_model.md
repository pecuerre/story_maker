...
[known_quirks.md](known_quirks.md)). `SectionsController` is the exception: a story starts
without section tags, so it falls back to `Story#default_section_tag`, which lazily creates a
tag named *"Section"*.
...
| `Section` | `name` presence; `story` required; parent rules scoped to the story; `section_tags` presence + all `section_tags` must belong to the section's story |
